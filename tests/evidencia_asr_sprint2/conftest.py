...
PAGO_DATE_OFFSET = 500  # reservas ASR05 empiezan en today+500 (>389 para evitar caches del run anterior)
...
def print_stats(asr_code: str, name: str, latencies_ms: list, threshold_ms: float):
    """Imprime la tabla de resultados estándar para un ASR."""
    if not latencies_ms:
        print(f"\n{asr_code} - {name}")
...
def reserva_dates(iteration: int, base_offset: int = BASE_DATE_OFFSET) -> tuple[str, str]:
