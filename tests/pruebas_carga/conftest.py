"""
Contexto compartido para las pruebas de carga ASR08 — TravelHub.

Autentica contra producción, descubre los IDs reales de recursos
(hotel, habitación, país, estado) y provee utilidades comunes.

Uso:
    from conftest import setup, BASE_URL, print_stats, percentile
...
# Offset en días desde hoy para las fechas de búsqueda de prueba.
BASE_DATE_OFFSET = 60
...
def print_stats(asr_code: str, name: str, latencies_ms: list, threshold_ms: float):
    ...
    line = "─" * 52
    print(f"\n{asr_code} - {name}")
    print(f"Muestras: {len(s)}  |  Umbral p95: {threshold_ms:.0f}ms")
    print(line)
...
def setup(verbose: bool = True) -> dict:
    ...
    Retorna un dict con:
        ...
        estado_id     — ID del primer estado disponible
    ...
    pendiente = next((e for e in estados if "pendiente" in e.get("nombre", "").lower()), None)
    estado_id = pendiente["id"] if pendiente else estados[0]["id"]
    if verbose:
        nombre_e = (pendiente or estados[0]).get("nombre", "?")
        print(f"  ✓ Estado      | id = {estado_id}  nombre = {nombre_e}")
